...
- **Monthly** — the year as a snaking ribbon of twelve month pills, three to a row, doubling back on itself. Events hang off stems above and below the track. Tap any month to open its full list underneath.
...
Four, switched in **Library → Look and feel** and remembered on the device:
...
| **Ink** | The original dark, for reading at night. |
...
The shell's height comes from `window.innerHeight`, written to `--app-h` in JS, with `100vh` and `100dvh` as fallbacks beneath it. This matters: **iOS reports a stale viewport height in standalone mode**, so a shell sized with `inset: 0` or `100vh` ends up short by the height of the Safari toolbar that is no longer there — which leaves the bottom nav floating above the bottom edge with dead space beneath it. `window.innerHeight` is correct there, and unlike `visualViewport.height` it does not shrink when the keyboard appears. It is re-measured on resize, rotation, `pageshow`, and on returning from the background.
