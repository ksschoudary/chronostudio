...
This is deliberate. A `position: fixed` bottom bar over a scrolling document drifts on iOS — it detaches during rubber-band overscroll and jumps when the keyboard opens. As a flex child of a pinned shell the nav has no way to move. Bottom sheets read `window.visualViewport` and lift above the keyboard via a `--kb` custom property rather than letting iOS shove the layout around.
