...
The bottom nav is gone: section navigation sits in the header between the lens bar and the filters. The shell is anchored on all four edges and `--app-h` is applied as a **min-height, never a fixed height** — a measurement can only correct a shell that came out too short, never impose a short one. Setting an explicit height here was itself the cause of dead space at the bottom in standalone: it overrode the correct bottom anchor with an under-reported value.

The height is the largest plausible candidate rather than any single API, because none is trustworthy on its own: iOS can under-report `window.innerHeight` in a freshly launched standalone app, `clientHeight` can lag, and `visualViewport.height` shrinks when the keyboard opens. The decisive candidate is `screen.height`, used **only** when `display-mode: standalone` matches — an installed app occupies the whole screen by definition, so that figure cannot be short, while in a browser tab it would be far too tall. The correction is capped at 1.3× the reported height so a bad reading can never push content off-screen.
