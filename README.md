...
**Planner** is where the app opens. A ticker across the top drifts through everything due today and over the next two days, pausing while touched; tapping the Today or Tomorrow tile opens the same three days as a centre popup. Below sits a full month grid — the same shape as the Timeline calendar, with plan titles in the date boxes and an overflow count where a day holds more than two — then four summary tiles (today, tomorrow, this month's count, and the total amount due), and the selected day's agenda.

Bills carry a status. Only *paid* is stored — **due** and **overdue** are derived from the date, so an unpaid bill becomes overdue on its own without anything running in the background. Paid bills recede and strike through their amount, overdue ones take a red tint, and the summary tiles show the overdue count and the total still unpaid. Repeating bills are marked paid one occurrence at a time, so paying this month's rent leaves next month's outstanding.

The briefing — today and the next two days — opens automatically on the first launch of each day, and only when something is actually due. A button below the grid reopens it any time, as do the Today and Tomorrow tiles. The date it last ran is kept in `chrono.briefed.v1`, so it appears once a day rather than on every launch.

A category filter sits under the ticker and narrows both the month grid and the day's agenda; the ticker itself ignores it, since it answers "what is coming" rather than "what am I looking at".

A plan carries a title, date, optional time, category (Event, Bill, Meeting, Birthday, Task, Holiday), location, repeat, reminder and notes. Bills additionally carry a type (Subscription, Utility, Business, Personal, Others) and an amount. Repeating plans are stored **once** and expanded on demand, so editing one changes every occurrence and nothing multiplies on disk — and completion is tracked per occurrence, so ticking this month's rent does not tick next month's.
...
In `localStorage` on that one device, under `chrono.events.v1`, `chrono.categories.v1`, `chrono.diary.v1`, `chrono.manifest.v1`, `chrono.habits.v1` and `chrono.planner.v1`. Consequences worth knowing:
